...
def run_reconx_agent(message_history: List[BaseMessage], user_id: int):
    ...
    """
    # config = {"recursion_limit": 5} # Fast hackathon limits
    inputs = {"messages": message_history, "user_id": user_id}
    
    # We call the graph. Higher recursion limits allow more tool use but take longer.
    # For a demo, 5-8 is plenty.
    result = reconx_agent.invoke(inputs, config={"recursion_limit": 8})
