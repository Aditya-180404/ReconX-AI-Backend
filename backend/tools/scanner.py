...
def run_scan_pipeline(target: str, scan_type: str = 'Quick') -> dict:
    ...
    from core.intel_engine import get_shodan_intel, get_vt_intel
    
    if cfg.get('shodan', True):
        out = get_shodan_intel(target)
        raw_parts.append(f"=== SHODAN INTEL ===\n{out}")
        # Parse critical Shodan findings
        if "[SHODAN] Identified CVEs:" in out:
            cves = out.split("[SHODAN] Identified CVEs:")[1].split('\n')[0].strip()
            findings.append({
                'severity': 'High',
                'title': f'Vulnerabilities Detected via Shodan: {cves[:50]}...',
                'endpoint': 'Network Interface',
                'fix': 'Update exposed services and restrict public access via firewall rules.'
            })
        if "Reliability: 0." in out and "Reliability: 0.0" not in out:
            findings.append({
                'severity': 'Medium',
                'title': 'High Honeypot Probability Detected',
                'endpoint': target,
                'fix': 'Target exhibits deceptive behavior. Proceed with caution during automated testing.'
            })

    if cfg.get('vt', True):
        out = get_vt_intel(target)
        raw_parts.append(f"=== VIRUSTOTAL INTEL ===\n{out}")
        if "ALERT: Host has been flagged as malicious" in out:
            findings.append({
                'severity': 'Critical',
                'title': 'Domain Flagged as Malicious (VirusTotal)',
                'endpoint': target,
                'fix': 'Investigate domain reputation and potential compromise.'
            })
