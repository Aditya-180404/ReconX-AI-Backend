...
def _scan_worker(scan_id, target, scan_type, user_id, custom_params=None):
    ...
    try:
        ...
        raw_logs = ""
        # Modern targets prefer HTTPS; tools will follow redirects if needed.
        # We ensure a protocol-qualified URL for tools like slowhttptest and nuclei.
        url = f"https://{target}" 
        ...
        tools = [
            ...
            ('headers',   'HTTP HEADERS',      ['curl', '-IkL', '--max-time', '15', url]),
            ('subfinder', 'SUBDOMAIN DISCOVERY', ['subfinder', '-d', target, '-silent']),
            ('nmap',      'PORT SCAN',          ['nmap', '-T4', '-F' if scan_type == 'Quick' else '-sV', '--open', target]),
            ('nikto',     'WEB SCRAPING',       ['nikto', '-h', url, '-nointeractive', '-Tuning', '123b']),
            ('sqlmap',    'SQL INJECTION',      ['sqlmap', '-u', url, '--batch', '--random-agent', '--level=1', '--forms', '--crawl=2', '--tamper=space2comment']),
            ('nuclei',    'VULN TEMPLATES',    ['nuclei', '-u', url, '-severity', 'critical,high,medium', '-silent', '-rl', '50']),
            ('zap',       'ZAP AUDIT',          ['zap-baseline.py', '-t', url, '-m', '1']),
            ('xss',       'XSS REFLECTION',     ['xsstrike', '-u', url, '--crawl', '--skip-dom']),
            ('dos',       'DOS RESILIENCE',     ['slowhttptest', '-c', '50', '-u', url, '-r', '100', '-t', 'GET', '-l', '20']),
        ]
